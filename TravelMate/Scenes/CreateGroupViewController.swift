...
class CreateGroupViewController: UIViewController {
    ...
    private func testBackendConnection() {
        Task {
            do {
                ...
                let url = URL(string: "\(Config.apiBaseURL)/groups")!
    ...
    private func updatePhotoButton(with image: UIImage) {
        photoButton.subviews.forEach { $0.removeFromSuperview() }
        ...
        NSLayoutConstraint.activate([
            imageView.topAnchor.constraint(equalTo: photoButton.topAnchor, constant: 8),
            imageView.leadingAnchor.constraint(equalTo: photoButton.leadingAnchor, constant: 8),
            imageView.trailingAnchor.constraint(equalTo: photoButton.trailingAnchor, constant: -8),
            imageView.bottomAnchor.constraint(equalTo: photoButton.bottomAnchor, constant: -8)
        ])
    }
    ...
    @objc private func createButtonTapped() {
        ...
        if isUploadingImage {
            print("❌ [CREATE] Image upload in progress - cannot create group yet")
            showErrorAlert(
                title: "Upload en cours",
                message: "Veuillez patienter pendant le téléversement de la photo du groupe."
            )
            return
        }
        ...
        print("   Target URL: \(Config.apiBaseURL)/groups")
        ...
        if provider.canLoadObject(ofClass: UIImage.self) {
            provider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
                ...
                Task {
                    ...
                    do {
                        let url = try await ImageUploadService.shared.uploadGroupImage(image)
                        await MainActor.run {
                        ...
                        await MainActor.run {
                            self.isUploadingImage = false
                            self.showErrorAlert(title: "Upload échoué", message: error.localizedDescription)
                        }
                    }
                }
            }
