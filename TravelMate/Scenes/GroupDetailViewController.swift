...
class GroupDetailViewController: UIViewController {
    ...
    // ✅ MÉTHODE CORRIGÉE : Afficher un seul bouton qui change d'état
    private func setupConditionalButtons() {
        // Vider tous les boutons existants
        ...
        if isCreator {
            // ✅ CRÉATEUR : Boutons Chat, Modifier, Demandes en attente, Supprimer
            let chatButton = createButton(
                title: "Ouvrir le chat du groupe",
                color: .systemBlue,
                action: #selector(openChatButtonTapped)
            )
            
            let modifyButton = createButton(
                title: "Modifier",
                color: .systemBlue,
                action: #selector(modifyButtonTapped)
            )
            
            let pendingButton = createButton(
                title: "Demandes en attente",
                color: .systemOrange,
                action: #selector(viewPendingRequestsTapped)
            )
            ...
            buttonsStackView.addArrangedSubview(pendingButton)
            buttonsStackView.addArrangedSubview(deleteButton)
            
        } else if isMember {
            // ✅ MEMBRE ACTIF : Boutons Chat + Quitter
            let chatButton = createButton(
                title: "Ouvrir le chat du groupe",
                color: .systemBlue,
                action: #selector(openChatButtonTapped)
            ...
            // ✅ NON MEMBRE : Vérifier si en attente de manière asynchrone
            // Afficher temporairement un indicateur de chargement
            let loadingButton = createButton(
                title: "Chargement...",
                color: .systemGray,
                action: nil
            )
            loadingButton.isEnabled = false
            buttonsStackView.addArrangedSubview(loadingButton)
            
            Task {
                let isPending = await currentGroup.isPending(userId: currentUserId)
                
                await MainActor.run {
                    // Retirer le bouton de chargement
                    self.buttonsStackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
                    
                    if isPending {
                        // ✅ DEMANDE EN ATTENTE : Bouton désactivé orange
                        let pendingButton = self.createButton(
                            title: "⏳ En attente d'approbation",
                            color: .systemOrange,
                            action: nil
                        )
                        pendingButton.isEnabled = false
                        pendingButton.alpha = 0.7
                        self.buttonsStackView.addArrangedSubview(pendingButton)
                    } else {
                        // ✅ PEUT REJOINDRE : Bouton vert actif
                        let joinButton = self.createButton(
                            title: "Rejoindre",
                            color: .systemGreen,
                            action: #selector(self.joinButtonTapped)
                        )
                        self.buttonsStackView.addArrangedSubview(joinButton)
                    }
                }
            }
        }
    ...
    @objc private func openChatButtonTapped() {
        let chatVC = GroupChatViewController(groupId: currentGroup.id, groupName: currentGroup.nom)
    ...
    // ✅ NOUVELLE ACTION : Voir les demandes en attente
    @objc private func viewPendingRequestsTapped() {
        let pendingVC = PendingRequestsViewController(groupId: currentGroup.id)
    ...
    // ✅ MÉTHODE MODIFIÉE : Après avoir rejoint, rafraîchir pour afficher "En attente"
    @objc private func joinButtonTapped() {
        ...
        Task {
            do {
                ...
                let _ = try await groupService.joinGroup(id: currentGroup.id)
                
                await MainActor.run {
                    // ✅ Afficher immédiatement "En attente" sans recharger depuis le serveur
                    self.buttonsStackView.arrangedSubviews.forEach { $0.removeFromSuperview() }
                    ...
                    self.showSuccessAlert(message: "✅ Demande envoyée ! Le créateur va l'examiner.")
                }
            } catch {
                await MainActor.run {
                    // En cas d'erreur, rétablir les boutons
                    self.setupConditionalButtons()
                    self.showErrorAlert(message: "❌ Erreur : \(error.localizedDescription)")
                }
            }
        }
    }
