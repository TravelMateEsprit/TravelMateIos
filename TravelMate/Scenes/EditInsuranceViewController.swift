...
protocol EditInsuranceDelegate: AnyObject {
    func didUpdateInsurance()
}

class EditInsuranceViewController: UIViewController {
    weak var delegate: EditInsuranceDelegate?
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            saveButton.topAnchor.constraint(equalTo: activeSwitch.bottomAnchor, constant: 30),
    ...
    private func populateFields() {
        nameTextField.text = insurance.name
        descriptionTextView.text = insurance.description
        priceTextField.text = String(insurance.price)
        durationTextField.text = insurance.duration
        coverageTextView.text = insurance.coverage.joined(separator: "\n")
        imageUrlTextField.text = insurance.imageUrl
        activeSwitch.isOn = insurance.isActive
    }
    ...
    private func validateInputs() -> Bool {
        guard let name = nameTextField.text, !name.isEmpty else {
            displayError(message: "Veuillez entrer un nom")
            return false
        }
        
        guard !descriptionTextView.text.isEmpty else {
            displayError(message: "Veuillez entrer une description")
            return false
        }
        
        guard let priceText = priceTextField.text, !priceText.isEmpty,
              let price = Double(priceText), price > 0 else {
            displayError(message: "Veuillez entrer un prix valide")
            return false
        }
        
        guard let duration = durationTextField.text, !duration.isEmpty else {
            displayError(message: "Veuillez entrer une durée")
            return false
        }
        
        guard !coverageTextView.text.isEmpty else {
            displayError(message: "Veuillez entrer au moins une couverture")
            return false
        }
