...
class GroupService {
    ...
    private let groupsCacheKey = "com.travelmate.groups.cache"
    ...
    // MARK: - Cache
    private func saveGroupsToCache() {
        do {
            let data = try JSONEncoder().encode(groups)
            UserDefaults.standard.set(data, forKey: groupsCacheKey)
        } catch {
            print("❌ [GROUP SERVICE] Failed to cache groups: \(error)")
        }
    }
    
    private func loadGroupsFromCache() {
        guard let data = UserDefaults.standard.data(forKey: groupsCacheKey) else { return }
        do {
            let cachedGroups = try JSONDecoder().decode([Group].self, from: data)
            groups = cachedGroups
            print("✅ [GROUP SERVICE] Loaded \(cachedGroups.count) groups from cache")
        } catch {
            print("❌ [GROUP SERVICE] Failed to load cached groups: \(error)")
        }
    }
    ...
    func fetchGroups() async {
        ...
        print("📍 [GROUP SERVICE] Endpoint: /groups")
        isLoading = true
        errorMessage = nil
    ...
    func fetchGroup(id: String) async throws -> Group {
        ...
        print("📍 [GROUP SERVICE] Endpoint: /groups/\(id)")
    ...
    func createGroup(_ dto: CreateGroupDto) async throws -> Group {
        ...
        do {
            let urlString = Config.apiBaseURL + "/groups"
            print("📡 [GROUP SERVICE] Appel POST: \(urlString)")
            if let bodyData = try? JSONEncoder().encode(dto),
               let bodyString = String(data: bodyData, encoding: .utf8) {
                print("📦 [GROUP SERVICE] Body: \(bodyString)")
            }
    ...
    func joinGroup(id: String) async throws -> Group {
        ...
        print("📍 [GROUP SERVICE] Endpoint: /groups/\(id)/join")
    ...
    func leaveGroup(id: String) async throws -> Group {
        ...
        print("📍 [GROUP SERVICE] Endpoint: /groups/\(id)/leave")
    ...
    func getPendingRequests(groupId: String) async throws -> [GroupMember] {
        print("🚀 [GROUP SERVICE] getPendingRequests called for group: \(groupId)")
        print("📍 [GROUP SERVICE] Endpoint: /groups/\(groupId)/pending-requests")
        
        do {
            let requests: [GroupMember] = try await networkService.request(
                endpoint: "/groups/\(groupId)/pending-requests",
                method: .get,
                requiresAuth: true
            )
            print("✅ [GROUP SERVICE] Found \(requests.count) pending requests")
            return requests
        } catch {
            print("❌ [GROUP SERVICE] Error fetching pending requests:")
            print("   - Error: \(error)")
            throw error
        }
    }
    
    func approveMemberRequest(groupId: String, userId: String) async throws {
        print("🚀 [GROUP SERVICE] approveMemberRequest called")
        print("📍 [GROUP SERVICE] Endpoint: /groups/\(groupId)/members/\(userId)/approve")
        
        do {
            let _: EmptyResponse = try await networkService.request(
                endpoint: "/groups/\(groupId)/members/\(userId)/approve",
                method: .post,
                requiresAuth: true
            )
            print("✅ [GROUP SERVICE] Member approved successfully")
        } catch {
            print("❌ [GROUP SERVICE] Error approving member:")
            print("   - Error: \(error)")
            throw error
        }
    }
    
    func rejectMemberRequest(groupId: String, userId: String) async throws {
        print("🚀 [GROUP SERVICE] rejectMemberRequest called")
        print("📍 [GROUP SERVICE] Endpoint: /groups/\(groupId)/members/\(userId)/reject")
        
        do {
            let _: EmptyResponse = try await networkService.request(
                endpoint: "/groups/\(groupId)/members/\(userId)/reject",
                method: .post,
                requiresAuth: true
            )
            print("✅ [GROUP SERVICE] Member rejected successfully")
        } catch {
            print("❌ [GROUP SERVICE] Error rejecting member:")
            print("   - Error: \(error)")
            throw error
        }
    }
